...
def img_to_array(path:str):
    ...
    # the image is arr_0 and label(388) is arr_1 # which is gaint panda in image net
    arr = np.array([])
    ...
    np.savez(path+"/pack.npz", arr, np.array([388]*len(arr)))
    return
...
def array_to_img(path:str):
    ...
    for i in range(len(samples)):
        plt.imsave(f"E:\\research\\MLLM\\src\\image\\denoised\\{i}.bmp", samples[i])
    return

if __name__ == "__main__":
    # img_to_array(".\\src\\image\\adversarial")
