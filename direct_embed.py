...
DEBUG = False
MODEL_PATH = "E:/LlaVA_hf/llava-1.5-7b-hf"
DEVICE = "cpu"
...
if DEBUG:
    ...
    image = Image.open("./image/clean.jpeg")
    ...
    # get image embedding
    pixel_value = imgprocessor(image, return_tensors="pt").pixel_values
    image_outputs = model.vision_tower(pixel_value, output_hidden_states=True)
    # hidden states has 25 layers
...
with open(f"{source_dir}/text/{text_malicious_file}", "r") as f:
    reader = csv.reader(f)
    for row in reader:
        # TODO only keep FunctionalCategory=standard rows
        text = row[0]
...
torch.save(malicious_text_embed_list, f"{source_dir}/embedding/{text1_save_filename}.pt")
torch.save(benign_text_embed_list, f"{source_dir}/embedding/{text2_save_filename}.pt")
